...
import com.example.backend.dto.ContributionReportRowDTO;
...
public class ContributionReportsGrid extends VerticalLayout {
    public ContributionReportsGrid(){
        ...
        grid.addColumn(ContributionReportRowDTO::period).setHeader("Periode");
        grid.addColumn(ContributionReportRowDTO::sheetNumber).setHeader("Num. full");
        grid.addColumn(ContributionReportRowDTO::irpfNumber).setHeader("Num. IRPF");
        grid.addColumn(ContributionReportRowDTO::submissionDate).setHeader("Data Presentacio");
        grid.addColumn(ContributionReportRowDTO::total).setHeader("Total");
        grid.addColumn(ContributionReportRowDTO::payment).setHeader("Pagament");
        grid.addColumn(ContributionReportRowDTO::status).setHeader("Estat");
        grid.addComponentColumn(contribution ->{
            Button viewBtn = new Button(VaadinIcon.EYE.create(), event ->{
                VaadinSession.getCurrent().setAttribute("contributionDTO", contribution);
